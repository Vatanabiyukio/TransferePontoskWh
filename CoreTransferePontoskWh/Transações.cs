...
namespace CoreTransferePontoskWh
{
    public class Transações
    {
        ...
        public static bool TransferirBase()
        {
            ...
            {
                Console.WriteLine("[!] Desculpe, é necessário que haja, ao menos, 2 pessoas cadastradas para realizar tal operação");
                return false;
            }
            ...
            else
            {
                ...
                if (Lista.PesquisarListaBase(baseEscolhida) != -1)
                {
                    ...
                    if (tipo)
                    {
                        ...
                        {
                            Console.WriteLine($"[!] A pessoa fisica \"{pessoaOrigem}\" não é a dona da base em \"{baseEscolhida}\"!");
                            Console.WriteLine("[!] Desculpe, operação cancelada!");
                            return false;
                        }
                    }
                    else
                    {
                        if (Lista.ListaPessoaJuridica[Lista.PesquisarListaPessoaJuridica(pessoaOrigem)].Cnpj !=
                            Lista.ListaBase[Lista.PesquisarListaBase(baseEscolhida)].Dono)
                        {
                            Console.WriteLine($"[!] A pessoa juridica \"{pessoaOrigem}\" não é a dona da base em \"{baseEscolhida}\"!");
                            Console.WriteLine("[!] Desculpe, operação cancelada!");
                            return false;
                        }
                    }
        ...
        public static bool CadastrarBase()
        {
            ...
            {
                Console.WriteLine("[!] Não nenhuma pessoa cadastrada. Por favor, registre alguém para então cadastrar uma base!");
                Console.WriteLine("[!] Desculpe, operação cancelada!");
                return false;
            }
            Console.WriteLine("[*] Por favor, insira os dígitos do CPF/CNPJ do dono da propriedade:");
            string donoIdentifação = Console.ReadLine();
            ...
            Base baseGerada = new Base(donoIdentifação, endereço);
            if (donoIdentifação is {Length: 11})
            {
        ...
        public static bool DescadastrarBase()
        {
            ...
            {
                Console.WriteLine($"[!] A propriedade em \"{endereço}\" foi confiscada da Pessoa Física \"{Lista.ListaBase[Lista.PesquisarListaBase(endereço)].Dono}\"!");
